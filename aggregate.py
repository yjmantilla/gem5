...
fig=bars(df_profile)
fig.savefig(os.path.join(OUTPUTPATH,'profiling.pdf'))
fig.savefig(os.path.join(OUTPUTPATH,'profiling.png'))
#plt.show()
...
def plot_optimal_points_with_results_by_workload(df,variables = ['wl', 'l1is', 'l2l', 'l3s', 'issw', 'robe', 'numfA'],targets=['system.cpu.cpi', 'Energy'],scaled=True, threshold=0.5,colors = {'h264dec': 'red', 'jpeg2k_enc': 'green', 'mp3_enc': 'blue'}, circle_sizes = [120, 80, 50]):
    ...
    fig, axes = plt.subplots(nrows=len(workloads), figsize=(6, 12))
    for ax, workload in zip(axes, workloads):
        ...
        for best_wl, best_row in best_points_constrained.iterrows():
            ...
            if scaled:
                ...
                ax.scatter(matching_point['original_'+targets[0]], matching_point['original_'+targets[1]], s=circle_sizes[best_wl], facecolors='none', edgecolors=colors[best_row[wl_variable]], linewidth=1.5, marker='o')
...
results,fig = plot_optimal_points_with_results_by_workload(df,variables=var_columns.tolist(),targets=targets,scaled=False, threshold=0.5,colors = {'h264dec': 'magenta', 'jpeg2k_enc': 'lime', 'mp3_enc': 'yellow'}, circle_sizes = [120, 90, 50])
